...
impl Editor {
    ...
    fn draw_status_bar(&self) {
        ...
        let modified_indiciator = if self.document.is_dirty() {
            "(modified)"
        } else {
            ""
        };
    ...
    fn search(&mut self) {
        let old_pos = self.cursor_position.clone();
        ...
        if let Some(query) = self
            ...
            .unwrap_or(None)
        {
            if let Some(position) = self.document.find(&query[..], &old_pos) {
    ...
    fn draw_welcome_message(&self) {
        let mut welcome_message = format!("R editor -- version {VERSION}");
        let width = self.terminal.size().width as usize;
    ...
    where
        // Function takes in The editor, a key, and the query
